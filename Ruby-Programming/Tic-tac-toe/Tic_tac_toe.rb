...
module TicTacToe
  ...
  class Game
    ...
    def initialize(player_1_class, player_2_class)
      @board = Array.new(10) # Index 0 is ignored for convenience
      @current_player_id = 0
    ...
    def place_player_marker(player)
      ...
      puts "#{player} selects #{player.marker} in position #{position}"
  ...
  class ComputerPlayer < Player
    ...
    def corner_trap_defense_position(_opponent_marker)
      log_debug 'defending against corner start by playing adjacent'
      opponent_position = @game.border.find_index { |marker| marker == opponent.marker }
      safe_responses = { 1 => [2, 4], 3 => [2, 6], 7 => [4, 8], 9 => [6, 8] }
      safe_responses[opponent_position].sample
    end

    def random_prioritized_position
      log_debug 'picking random position, favoring center and then the corners'
